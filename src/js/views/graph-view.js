...
function getYourEstimates() {
  ...
  $.when(
    ...
    if ( resp.error === '' ) {
      ...
      textlets.currentAge = window.gettext( SSData.currentAge );
      textlets.fullRetirementAge = window.gettext(
...
function setAgeWithIndicator( indicatorValue ) {
  ...
  textlets.selectedAge = window.gettext( selectedAge );
...
function getTranslations() {
  for ( const key in textlets ) {
    if ( {}.hasOwnProperty.call( textlets, key ) ) {
      textlets[key] = window.gettext( textlets[key] );
    }
  }
}
